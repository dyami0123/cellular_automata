...
impl GGEZShapeRenderer<'_> {
    pub fn draw_shape(self, shape: &dyn shapes::shape::Shape) -> GenericResult {
        match shape.shape_type() {
            shapes::shape_types::ShapeType::SinglePoint => {
                if let Some(point_shape) = shape
                    .as_any()
                    .downcast_ref::<shapes::single_point_shape::SinglePointShape>(
                ) {
                    return self.draw_point(
                        point_shape.points()[0],
                        point_shape.size(),
                        point_shape.appearance(),
                    );
                } else {
            ...
            shapes::shape_types::ShapeType::SingleLine => {
                if let Some(line_shape) = shape
                    .as_any()
                    .downcast_ref::<shapes::single_line_shape::SingleLineShape>()
                {
                    return self.draw_line(
                        line_shape.points()[0],
                        line_shape.points()[1],
                        line_shape.size(),
                        line_shape.appearance(),
                    );
                } else {
                    // Handle the case where downcasting fails
                    return Err(std::fmt::Error);
            ...
            shapes::shape_types::ShapeType::Rectangle => {
                if let Some(rectangle_shape) = shape
                    .as_any()
                    .downcast_ref::<shapes::rectangle_shape::RectangleShape>()
                {
                    return self.draw_rectangle(
                        rectangle_shape.points()[0],
                        rectangle_shape.points()[1],
                        rectangle_shape.size(),
                        rectangle_shape.appearance(),
                    );
                } else {
                    // Handle the case where downcasting fails
                    return Err(std::fmt::Error);
            ...
            shapes::shape_types::ShapeType::Triangle => {
                if let Some(triangle_shape) = shape
                    .as_any()
                    .downcast_ref::<shapes::triangle_shape::TriangleShape>()
                {
                    return self.draw_triangle(
                        triangle_shape.points()[0],
                        triangle_shape.points()[1],
                        triangle_shape.points()[2],
                        triangle_shape.size(),
                        triangle_shape.appearance(),
                    );
                } else {
                    // Handle the case where downcasting fails
                    return Err(std::fmt::Error);
...
impl Renderer for GGEZShapeRenderer<'_> {
    fn draw_point(self, start: (f32, f32), width: f32, appearance: Appearance) -> GenericResult {
    ...
    fn draw_rectangle(
        ...
        let rectangle_game_result = graphics::Mesh::new_rectangle(
            self.ctx,
            graphics::DrawMode::fill(),
            graphics::Rect::new(start.0, start.1, end.0 - start.0, end.1 - start.1),
            Color::from(appearance.color),
        );
